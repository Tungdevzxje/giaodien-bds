...
<h3 style="font-size:0px; margin: 0px; height:0px; color:#fff; margin: 0px; padding: 0px;"><a>xưởng in ấn phẩm văn phòng
        đà nẵng</a></h3>
<h3 style="font-size:0px; margin: 0px; height:0px; color:#fff; margin: 0px; padding: 0px;"><a>xưởng in kỷ niệm chương đà
        nẵng</a>
</h3>
<h3 style="font-size:0px; margin: 0px; height:0px; color:#fff; margin: 0px; padding: 0px;"><a>xưởng in ấn phẩm quà tặng
        đà nẵng</a></h3>
...
<div class="page-content">
            <!-- INNER PAGE BANNER -->
            <div class="wt-bnr-inr overlay-wraper bg-parallax bg-top-center"  data-stellar-background-ratio="0.5"  style="background-image:url(hinhmenu/banner-tintuc.jpg);">
                    ...
                    <div class="wt-bnr-inr-entry">
                    	<div class="banner-title-outer">
                        	<div class="banner-title-name">
                        		<h2 class="text-white text-uppercase letter-spacing-5 font-18 font-weight-300">ELY ARCHI CONSTRUCTION</h2>
                            </div>
                        </div>
                        <!-- BREADCRUMB ROW -->
                        <div class="p-tb20">
                            <div>
                                <?php
                                $id = $_GET['url'];
                                $tv = "select * from tin_dichvuabcd where linkurl like '%" . $id . "%' order by id ";
                                $tv_1 = mysqli_query($link, $tv);
                                $tv_2 = mysqli_fetch_array($tv_1);
                                $thuocloai = "$tv_2[thuocloai]";
                                $phanloai = mysqli_fetch_array(mysqli_query($link, "select * from  loai_tin_dichvuabcd where id=" . $thuocloai . " limit 1"));
                                $mota = "$tv_2[mota]";
                                $hinhanh = "$tv_2[hinhanh]";
                                $hinhanh1 = "$tv_2[hinhanh1]";
                                $hinhanh2 = "$tv_2[hinhanh2]";
                                $hinhanh3 = "$tv_2[hinhanh3]";
                                $mysql = mysqli_query($link, "select * from loai_tin_dichvuabcd where id=$thuocloai limit 1");
...
<section class="blog bg-white">
    <div class="container" id="pinBoxContainer">
        <div class="row">
            <div class="col-lg-9 col-md-12 blog-pots">
                ...
                $id = $_REQUEST["url"];
                $result = mysqli_query($link, "SELECT * FROM tin_dichvuabcd where linkurl like '%" . $id . "%' order by id desc limit $start,$limit");
                // hiển thị DL
    ...
    padding-left: 5px;"><i class="fa fa-bars" style="padding-right:10px;"></i> TIN LIÊN QUAN </div>
                    ...
                    <div class="row">
                        ...
                        $limit = 6;
                        $start = $p->findStart($limit);
                        $count_query = mysqli_query($link, "SELECT * FROM tin_dichvuabcd WHERE thuocloai = " . $thuocloai);
                        $count = mysqli_num_rows($count_query);
                        $pages = $p->findPages($count, $limit);
                        $sql = mysqli_query($link, "SELECT * FROM (SELECT * FROM tin_dichvuabcd WHERE thuocloai = $thuocloai ORDER BY id DESC LIMIT 100) AS latest_20 ORDER BY RAND() LIMIT 6");
                        // Error handling
                        if (!$sql) {
    ...
    text-transform: none;
    padding: 10px;
    line-height: 25px;"><a href="<?php echo "$link" . '-' . $tv_2['id']; ?>" style="line-height:22px;"><?php echo $tieude; ?> </a></h3>
...
<!-- END SECTION BLOG -->
<!-- Start Blog Area -->
<section class="eblog-bottom-post-area tp-section-gap blog" style='padding:0px;'>
    <div class="container">
        <div style="border-bottom: 2px solid #000;
    padding: 7px;
    color: #c00;
                                    ...
                                    <div class="eblog-featured-news style-two small">
                                        ...
                                        <div class="blog-content text-left">
                                            ...
                                            <a href="<?php echo $postLink; ?>">
                                                <div class="heading-title ml--0 mb--10 text-start" style="    color: #222;
    font-size: 17px;
    font-weight: 800;
    text-transform: none;
    padding: 10px;
    line-height: 25px;"><?php echo $postTitle; ?></div>
                                            </a>
